...
function loadMentors() {
  ...
  _mentors = lines.slice(1).map(line => {
    ...
    const countries = [];
    if (/japan|tokyo|도쿄|일본|lionice|라이오니스|k-startup center/.test(orgLower)) countries.push('🇯🇵 일본');
    ...
    return { name: obj['이름'] || '', org: obj['소속'] || '', title: obj['직위'] || '', fields, industries, years, countries };
  }).filter(m => m.name && m.name !== '한동환');
...
function matchMentors(topFields, companyIndustries = [], topN = 5) {
  ...
  if (scored.length < topN) {
    ...
    const fallback = mentors.filter(m => !seen.has(m.name)).sort((a, b) => b.years - a.years);
